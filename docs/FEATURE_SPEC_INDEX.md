...
| Immutable policy epoch and label provenance | shipped measurement safety; legacy-rescan laundering blocked | `docs/specs/policy-epoch-label-provenance.md` | verified-row coverage, epoch separation, temporal holdout integrity | accumulate genuinely new provenance-verified labels before ranker retraining/promotion |
...
| Canonical unified portfolio alpha | measurement-only shipped; local maximum-available replay restored | `docs/specs/canonical-portfolio-alpha.md` | net portfolio return after costs, BTC benchmark return, `net_alpha_after_costs`, drawdown | current 30d baseline is decision-grade but deeply negative; use paired frozen-cache comparisons before any trading promotion |
