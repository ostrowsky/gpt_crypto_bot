...
| Canonical unified portfolio alpha | measurement-only shipped; explicit maximum-available cutoff supported | `docs/specs/canonical-portfolio-alpha.md` | net portfolio return after costs, BTC benchmark return, `net_alpha_after_costs`, drawdown | restore complete market-data coverage, then rerun anchored 30d ten-slot artifact; `pnl_total` is diagnostic-only |
